from datamenity.models import session_scope, HotelReview, HotelReviewSentiment, OTAType, Environment, CompareHasTag, Compare
...
def batch_roomtype():
    with session_scope() as db_session:
        env_item = db_session.query(Environment).filter(Environment.key == 'ROOMTYPE_RULE').first()
        if env_item is None:
            raise Exception('room_type 설정이 존재하지 않음')

        # roomtype
        set_roomtype_hotel_rule_whole([], json.loads(env_item.value))
        set_compare_has_tag_whole([])

        # 중복 제거
        anomal_compares = db_session.query(Compare).filter(Compare.priority.is_(None)).all()
        anomal_compare_ids = [a.id for a in anomal_compares]
        db_session.query(CompareHasTag).filter(CompareHasTag.compare_id.in_(anomal_compare_ids)).delete()
        db_session.query(Compare).filter(Compare.priority.is_(None)).delete()
        db_session.commit()
...
def batch():
    ...
    if (kst_now.hour != 1 or kst_now.minute != 0) and (kst_now.hour != 12 or kst_now.minute != 0):
        return
